...
class HomeRemoteDataSourceImpl implements HomeRemoteDataSource {
  ...
  @override
  Future<Either<Failure, AddToCartResponseDto>> addToCart(String productId) async {
    try {
      ConnectivityResult checkResult = ConnectivityResult.wifi;
      if (checkResult == ConnectivityResult.wifi || checkResult == ConnectivityResult.mobile) {
  ...
  @override
  Future<Either<Failure, GetCartResponseDto>> updateNumOfCartInProductDetails(String productId, int count) async {
    try {
      ConnectivityResult checkResult = ConnectivityResult.wifi;
      if (checkResult == ConnectivityResult.wifi || checkResult == ConnectivityResult.mobile) {
        var token = SharedPrefUtils.getDate(key: 'token');
        var response = await apiManger.updateData('${EndPoints.cart}/$productId',
            body: {
              'count': '$count'
            },
            headers: {
  ...
  @override
  Future<Either<Failure, GetWishListResponseDto>> addToWishList({required String productId}) async{
    try {
      ConnectivityResult checkResult = ConnectivityResult.wifi;
      if (checkResult == ConnectivityResult.wifi || checkResult == ConnectivityResult.mobile) {
        var token = SharedPrefUtils.getDate(key: 'token');
        ...
        var addToWishListResponse = GetWishListResponseDto.fromJson(response.data);
        if (response.statusCode! >= 200 && response.statusCode! < 300) {
