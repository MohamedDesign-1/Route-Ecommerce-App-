...
class WishListRemoteDataSourceImpl implements WishListRemoteDataSource {
  ...
  Future<Either<Failure, GetWishListResponseDto>> getWishList() async {
    try {
      ...
      if (checkResult == ConnectivityResult.wifi || checkResult == ConnectivityResult.mobile) {
        ...
        var response = await apiManger.getData(EndPoints.wishlist,
            headers: {
              'token': token.toString()
            });
        var getWishListResponse = GetWishListResponseDto.fromJson(response.data);
  ...
  @override
  Future<Either<Failure, String>> deleteProductFromWishList({required String productId}) async{
    try {
      ConnectivityResult checkResult = ConnectivityResult.wifi;
      if (checkResult == ConnectivityResult.wifi || checkResult == ConnectivityResult.mobile) {
        var token = SharedPrefUtils.getDate(key: 'token');
        var response = await apiManger.deleteData('${EndPoints.wishlist}/$productId',
            headers: {
              'token': token.toString()
            });
        if (response.statusCode! >= 200 && response.statusCode! < 300) {
