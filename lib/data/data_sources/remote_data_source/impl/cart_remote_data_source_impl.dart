...
class CartRemoteDataSourceImpl implements CartRemoteDataSource {
  ...
  Future<Either<Failure, GetCartResponseDto>> getCart() async {
    try {
      ...
      if (checkResult == ConnectivityResult.wifi || checkResult == ConnectivityResult.mobile) {
        ...
        var response = await apiManger.getData(EndPoints.cart,
            headers: {
  ...
  @override
  Future<Either<Failure, GetCartResponseDto>> deleteItemInCart(String productId) async {
    try {
      ConnectivityResult checkResult = ConnectivityResult.wifi;
      if (checkResult == ConnectivityResult.wifi || checkResult == ConnectivityResult.mobile) {
        var token = SharedPrefUtils.getDate(key: 'token');
  ...
  @override
  Future<Either<Failure, GetCartResponseDto>> updateCountInCart(String productId, int count) async {
    try {
      ConnectivityResult checkResult = ConnectivityResult.wifi;
      if (checkResult == ConnectivityResult.wifi || checkResult == ConnectivityResult.mobile) {
        var token = SharedPrefUtils.getDate(key: 'token');
