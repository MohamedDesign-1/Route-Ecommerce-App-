    ...
    return BlocConsumer<ProductViewModel, ProductState>(
      ...
      listener: (context, state) {
        if (state is AddToWishlistSuccessState) {
          ToastUtils.showSuccessToast(AppConstants.success, context, AppConstants.addToWishList);
        } else if (state is AddToWishlistErrorState) {
          ToastUtils.showErrorToast(AppConstants.error, context, AppConstants.somethingWentWrong);
        }
        if (state is AddToCartSuccessState) {
          ToastUtils.showSuccessToast(AppConstants.success, context, AppConstants.addToCartList);
                            ...
                            itemBuilder: (context, index) => InkWell(
                                onTap: () {
                                  Navigator.push(context, MaterialPageRoute(builder: (context) =>
                                              ProductDetials(productEntity: ProductViewModel.get(context).productList[index],
                                              )));
                                },
                                child: CustomProductItem(
                                  product: ProductViewModel.get(context).productList[index],
                                )),
                            itemCount: ProductViewModel.get(context).productList.length),
                      ),
                    ],
                  ),
                )
