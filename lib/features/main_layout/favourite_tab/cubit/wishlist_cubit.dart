...
class WishlistCubit extends Cubit<WishlistState> {
  ...
  WishlistCubit({required this.getWishListUseCase , required this.addToWishListUseCase , required this.deleteFromWishListUseCase}) : super(WishlistInitial());
  ...
  void getWishList() async {
    Either<Failure, GetWishListResponseEntity> result = await getWishListUseCase.invoke();
    result.fold((failure) {
  ...
  void deleteFromWishList(String productId) async {
    ...
    result.fold((failure) {
      ...
      emit(WishlistSuccessState(responseEntity: GetWishListResponseEntity(data: productList)));
