        ...
        body: BlocConsumer<WishlistCubit, WishlistState>(
          listener: (context, state) {
            if (state is DeleteFromWishlistSuccessState) {
              ToastUtils.showSuccessToast(AppConstants.success, context, AppConstants.deleteFromWishList);
          ...
          builder: (context, state) {
            if (WishlistCubit.get(context).productList.isEmpty) {
              return NotHaveItems(icon: Icons.favorite_border_outlined, message: AppConstants.yourWishListIsEmpty,);
            } else if (state is WishlistSuccessState  || state is DeleteFromWishlistSuccessState) {
              return Column(
                children: [
                  Expanded(
                    child: ListView.separated(
                      ...
                      itemBuilder: (context, index) {
                        return WishListItemCard(index: index,);
                      },
                    ),
                  ),
                ],
