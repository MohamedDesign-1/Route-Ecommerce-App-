    ...
    body: Padding(
      ...
      child: SingleChildScrollView(
        child: Column(
          children: [
            AdsWidget(),
            CustomSection(sectionName:'Categories', function: (){},),
            ...
            SizedBox(
              ...
              itemBuilder: (context, index) => CustomCategoryWidget(list: HomeTabViewModelCubit.get(context).categoryList[index],),
              itemCount: HomeTabViewModelCubit.get(context).categoryList.length,
              ),
            ) : CircularProgressIndicator(),
            CustomSection(sectionName:'Brands', function: (){},),
            SizedBox(
              height: 270.h,
              child: GridView.builder(
                scrollDirection: Axis.horizontal,
                gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2),
                itemBuilder: (context, index) => CustomCategoryWidget(list: HomeTabViewModelCubit.get(context).brandsList[index],),
                itemCount: HomeTabViewModelCubit.get(context).brandsList.length,
                  ),
                )
              ],
            ),
