...
class CartViewModelCubit extends Cubit<CartViewModelState> {
  ...
  CartViewModelCubit({required this.getCartUseCase , required this.deleteItemUseCase , required this.updateCountInCartUseCase}) : super(CartViewModelInitial());
  ...
  void getCart() async {
    ...
    Either<Failure, GetCartResponseEntity> result = await getCartUseCase.invoke();
    result.fold((failure) {
  ...
  void deleteItemInCart(String productId) async {
    Either<Failure, GetCartResponseEntity> result = await deleteItemUseCase.invoke(productId);
    result.fold((failure) {
  ...
  void updateCountInCart(String productId , int count) async {
    Either<Failure, GetCartResponseEntity> result = await updateCountInCartUseCase.invoke(productId , count);
    result.fold((failure) {
